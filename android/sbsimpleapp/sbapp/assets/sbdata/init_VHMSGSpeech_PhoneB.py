...
class MyVHEngine(VHEngine):
	...
	def eventVoiceRecognition(self, success, recogText):
		if success:
			...
			self.send('sb saySomething("ChrTessa","' + recogText + '")')
		else:
...
setupCharacter("ChrJin", "ChrJin", "", "")
bml.execBML('ChrJin', '<body posture="ChrBrad@Idle01"/>')
bml.execBML('ChrJin', '<saccade mode="listen"/>')
