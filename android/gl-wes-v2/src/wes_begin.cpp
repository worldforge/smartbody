    ...
    *vt_current = *vt_const;
    ...
    for(i = 0; i < WES_ANUM; i++)
    {
        if (vt_attrib_pointer[i].isenabled){
            ...
            glVertexAttribPointer(i, vt_attrib_pointer[i].size, vt_attrib_pointer[i].type,
...
wes_vertbuffer_flush()
{
    ...
    glVertexAttribPointer(WES_APOS, vt_possize, GL_FLOAT, GL_FALSE, sizeof(vertex_t), vt_vbuffer);
    if (vt_coordsize[0]){
        glEnableVertexAttribArray(WES_ATEXCOORD0);
        glVertexAttribPointer(WES_ATEXCOORD0, vt_coordsize[0], GL_FLOAT, GL_FALSE, sizeof(vertex_t),
    ...
    if (vt_coordsize[1]){
        ...
        glVertexAttribPointer(WES_ATEXCOORD1, vt_coordsize[1], GL_FLOAT, GL_FALSE, sizeof(vertex_t),
            (GLfloat*)(vt_vbuffer) + WES_OFFSET_TEXCOORD1);
    } else {
        glDisableVertexAttribArray(WES_ATEXCOORD1);
    }
    if (vt_coordsize[2]){
        glEnableVertexAttribArray(WES_ATEXCOORD2);
        glVertexAttribPointer(WES_ATEXCOORD2, vt_coordsize[2], GL_FLOAT, GL_FALSE, sizeof(vertex_t),
            (GLfloat*)(vt_vbuffer) + WES_OFFSET_TEXCOORD2);
    } else {
        glDisableVertexAttribArray(WES_ATEXCOORD2);
    }
    if (vt_coordsize[3]){
        glEnableVertexAttribArray(WES_ATEXCOORD3);
        glVertexAttribPointer(WES_ATEXCOORD3, vt_coordsize[3], GL_FLOAT, GL_FALSE, sizeof(vertex_t),
            (GLfloat*)(vt_vbuffer) + WES_OFFSET_TEXCOORD3);
    } else {
        glDisableVertexAttribArray(WES_ATEXCOORD3);
    }
    if (vt_normalsize){
        glEnableVertexAttribArray(WES_ANORMAL);
        glVertexAttribPointer(WES_ANORMAL, vt_normalsize, GL_FLOAT, GL_FALSE, sizeof(vertex_t),
            (GLfloat*)(vt_vbuffer) + WES_OFFSET_NORMAL);
    } else {
        glDisableVertexAttribArray(WES_ANORMAL);
    }
    if (vt_fogcoordsize){
        glEnableVertexAttribArray(WES_AFOGCOORD);
        glVertexAttribPointer(WES_AFOGCOORD, vt_fogcoordsize, GL_FLOAT, GL_FALSE, sizeof(vertex_t),
            (GLfloat*)(vt_vbuffer) + WES_OFFSET_FOGCOORD);
    } else {
        glDisableVertexAttribArray(WES_AFOGCOORD);
    }
    if (vt_color0size){
        glEnableVertexAttribArray(WES_ACOLOR0);
        glVertexAttribPointer(WES_ACOLOR0, vt_color0size, GL_FLOAT, GL_FALSE, sizeof(vertex_t),
            (GLfloat*)(vt_vbuffer) + WES_OFFSET_COLOR0);
    } else {
        glDisableVertexAttribArray(WES_ACOLOR0);
    }
    if (vt_color1size){
        glEnableVertexAttribArray(WES_ACOLOR1);
        glVertexAttribPointer(WES_ACOLOR1, vt_color1size, GL_FLOAT, GL_FALSE, sizeof(vertex_t),
            (GLfloat*)(vt_vbuffer) + WES_OFFSET_COLOR1);
    } else {
        glDisableVertexAttribArray(WES_ACOLOR1);
    }
    ...
    if (vt_mode == GL_QUADS){
        ...
        glDrawElements(GL_TRIANGLES, num * 6, GL_UNSIGNED_SHORT, vt_ibuffer);
    } else {
        glDrawArrays(vt_mode, 0, vt_count);
    }
...
glBegin(GLenum mode)
{
    ...
    /* Set Constant data */
    glVertexAttrib4f(WES_APOS, vt_current->x, vt_current->y, vt_current->z, vt_current->w);
    glVertexAttrib4f(WES_ATEXCOORD0, vt_current->coord[0].s, vt_current->coord[0].t, vt_current->coord[0].r, vt_current->coord[0].q);
    glVertexAttrib4f(WES_ATEXCOORD1, vt_current->coord[1].s, vt_current->coord[1].t, vt_current->coord[1].r, vt_current->coord[1].q);
    glVertexAttrib4f(WES_ATEXCOORD2, vt_current->coord[2].s, vt_current->coord[2].t, vt_current->coord[2].r, vt_current->coord[2].q);
    glVertexAttrib4f(WES_ATEXCOORD3, vt_current->coord[3].s, vt_current->coord[3].t, vt_current->coord[3].r, vt_current->coord[3].q);
    glVertexAttrib3f(WES_ANORMAL, vt_current->nx, vt_current->ny, vt_current->nz);
    glVertexAttrib1f(WES_AFOGCOORD, vt_current->fog);
    glVertexAttrib4f(WES_ACOLOR0, vt_current->cr0, vt_current->cg0, vt_current->cb0, vt_current->ca0);
    glVertexAttrib3f(WES_ACOLOR1, vt_current->cr1, vt_current->cg1, vt_current->cb1);
    *vt_const = *vt_current;
...
glVertexPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *ptr)
{
    ...
    glVertexAttribPointer(WES_APOS, size, type, GL_FALSE, stride, ptr);
}
...
glNormalPointer(GLenum type, GLsizei stride, const GLvoid *ptr)
{
    ...
    glVertexAttribPointer(WES_ANORMAL, 3, type, GL_FALSE, stride, ptr);
}
...
glColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *ptr)
{
    ...
    glVertexAttribPointer(WES_ACOLOR0, size, type, GL_FALSE, stride, ptr);
}
...
glSecondaryColorPointer(GLint size, GLenum type, GLsizei stride, const GLvoid *ptr)
{
    ...
    glVertexAttribPointer(WES_ACOLOR1, size, type, GL_FALSE, stride, ptr);
}
...
glFogCoordPointer(GLenum type, GLsizei stride, const GLvoid *ptr)
{
    ...
    glVertexAttribPointer(WES_AFOGCOORD, 1, type, GL_FALSE, stride, ptr);
}
...
glEnableClientState(GLenum array)
{
    ...
    switch(array)
    {
        ...
        case GL_TEXTURE_COORD_ARRAY:
            glEnableVertexAttribArray(WES_ATEXCOORD0 + vt_clienttex);
...
glDisableClientState(GLenum array)
{
    ...
    {
        case GL_VERTEX_ARRAY:
            glDisableVertexAttribArray(WES_APOS);
        ...
        case GL_FOG_COORD_ARRAY:
            ...
            break;
        case GL_TEXTURE_COORD_ARRAY:
            glDisableVertexAttribArray(WES_ATEXCOORD0 + vt_clienttex);
...
glInterleavedArrays(GLenum format, GLsizei stride, const GLvoid *pointer)
{
    ...
    glEnableVertexAttribArray(WES_APOS);
    glVertexAttribPointer(WES_APOS, sv, GL_FLOAT, GL_FALSE, str, (GLubyte*)pointer + pv);
    if (et) {
        glEnableVertexAttribArray(WES_ATEXCOORD0 + vt_clienttex);
        glVertexAttribPointer(WES_ATEXCOORD0 + vt_clienttex, st, GL_FLOAT, GL_FALSE, str, pointer);
    } else {
        glDisableVertexAttribArray(WES_ATEXCOORD0 + vt_clienttex);
    }
    if (ec) {
        glEnableVertexAttribArray(WES_ACOLOR0);
        glVertexAttribPointer(WES_ACOLOR0, st, tc, GL_FALSE, str, (GLubyte*)pointer + pc);
    } else {
        glDisableVertexAttribArray(WES_ACOLOR0);
    }
    if (en) {
        glEnableVertexAttribArray(WES_ANORMAL);
        glVertexAttribPointer(WES_ANORMAL, 3, GL_FLOAT, GL_FALSE, str, (GLubyte*)pointer + pn);
    } else {
        glDisableVertexAttribArray(WES_ANORMAL);
    }
}
...
GLvoid
glDrawArrays_wes(GLenum mode, GLint off, GLint num)
{
    wes_vertbuffer_flush();
    wes_state_update();
    glDrawArrays(mode, off, num);
}

GLvoid 
glDrawElements_wes(GLenum mode, GLsizei count, GLenum type, const GLvoid *indices)
{
	wes_vertbuffer_flush();
	wes_state_update();
