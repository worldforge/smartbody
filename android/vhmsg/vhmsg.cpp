...
#if 1   // keeping stub code at the bottom of this file for platforms that don't support vhmsg
...
public:
   class InternalListener : public cms::MessageListener
   {
      ...
      public:
         InternalListener() { m_listener = NULL; m_client = NULL; }
...
{
   string arg_encoded = HtppUtility_UrlEncode( ( arg ) );//vhcl::WStringToAnsi( HtppUtility_UrlEncode( vhcl::AnsiToWString( arg ) ) );

   string mess = string( op ) + " " + arg_encoded;

   cms::TextMessage * message = pimpl->m_session->createTextMessage( mess );
...
bool Client::Subscribe( const char * req )
{
   ...
   pimpl->m_consumers.emplace_back( std::make_pair( req, consumer ) );
...
void Client::WaitAndPoll( const double waitTimeSeconds )
{
   ...
   timespec ts;
   ts.tv_sec = tb.time;
   ts.tv_nsec = tb.millitm * 1000000;  // millisecond to nanosecond
...
void ClientImpl::InternalListener::onMessage( const cms::Message * message ) throw()
{
   if ( m_listener )
   {
      ...
      string surl = HtppUtility_UrlDecode( s );

      m.SetString( surl.c_str() ); //vhcl::WStringToAnsi( ws ).c_str() );
       LOG("vhmsg str = %s\n",surl.c_str());
      ...
      for ( size_t i = 0; i < properties.size(); i++ )
      {
         ...
         wstring wpropertyString = vhcl::AnsiToWString(HtppUtility_UrlDecode( propertyString ));
#ifdef __ANDROID__
         m.InsertItem( String_Pair( properties[ i ], HtppUtility_UrlDecode( propertyString ) ) );
#else
         m.InsertItem( String_Pair( properties[ i ], vhcl::WStringToAnsi( wpropertyString ) ) );
#endif
