...
int ttu_notify( const char * opname, const int argc, const char ** argv )
{
   if ( g_vhmsg == NULL )
   {
...
int ttu_notify1( const char * args_in )
{
#ifdef DEBUG_BUILD
   Log( "tt_utils: Attempt ttu_notify1: \"%s\"", args_in );
#endif

   size_t args_len = strlen( args_in );
   char * args = new char[ args_len + 1 ];
   strcpy( args, args_in );


   const char* cmd      = strtok( args, " \t\r\n" );
...
#ifdef DEBUG_BUILD
   if( cmd_args )
      Log( "tt_utils: Command: \"%s\", Command Args:\"%s\"", cmd, cmd_args );
   else
      Log( "tt_utils: Command:\"%s\" (no args)", cmd );
#endif

   int result = ttu_notify2( cmd, cmd_args );
   delete [] args;
...
int ttu_notify2( const char * opname, const char * arg )
{
   if ( arg )
   {
      return ttu_notify( opname, 1, &arg );
   }

   return ttu_notify( opname, 0, NULL );
}
