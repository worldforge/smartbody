...
enum KeyWord {
	ROOT, OFFSET, CENTER, CHANNELS, CHANNEL, JOINT, END,
...
static KeyWord read_keyword(SrInput& in) {
	...
	else if (s == "offset") kw = OFFSET; // offset to position joints
	else if (s == "center") kw = CENTER; // same as offset, more natural for joint list mode
	else if (s == "channels") kw = CHANNELS; // bvh channels definition
	else if (s == "channel") kw = CHANNEL; // bvh channels definition
	else if (s == "joint") kw = JOINT; // joint declaration
	else if (s == "modelmat") kw = MODELMAT; // matrix to adjust geometries
	else if (s == "modelrot") kw = MODELROT; // matrix to adjust geometries
	else if (s == "prerot" || s == "framerot") kw = PREROT; // to pre-multiply joint rotations
	else if (s == "postrot") kw = POSTROT; // to post-multiply joint rotations
	else if (s == "align") kw = ALIGN; // adjusts pre/post to have 1st child with given offset
	else if (s == "visgeo") kw = VISGEO; // visualization geometry
	else if (s == "colgeo") kw = COLGEO; // collision geometry
	else if (s == "euler") kw = EULER; // euler order/type specification
	else if (s == "end") kw = END; // default string comparison is case-insensitive
...
static bool read_channels(SrInput& in, SkJoint* j, SrArray<SkChannel::Type>& channels) {
	...
	while (n-- > 0) {
		...
		c = channels.top();
		if (c <= SkChannel::ZPos) { j->pos()->limits(c, false); } // turn off dof limits
		else if (c <= SkChannel::ZRot) {
			j->euler()->limits(c - 3, false); // turn off dof limits
			j->euler()->activate();
		} else { j->quat()->activate(); }
	}
	return true;
}
...
static bool read_channel(SrInput& in, SkJoint* j) {
	...
	if (c == SkChannel::Twist) {
		...
		j->st()->twist(tw);
		in.get_token();
		if (in.last_token() == "free") { j->st()->twist_limits(false); }
		else {
	...
	if (c <= 5) // 3:XRot, 4:YRot, 5:ZRot
	{
		...
		j->euler()->activate();
		int d = c - 3;
		if (free) { j->euler()->limits(d, false); }
		else {
...
bool SkSkeleton::loadSk(SrInput& in, double skScale, const char* basedir) {
	...
	bool merge = false;
	in.get_token();
	if (in.last_token() == "joint_data") { merge = true; }
	else {
		in.unget_token();
		init();
	}
...
SkJoint* SkSkeleton::_loadjlist(SrInput& in, float scale, SrStringArray& paths) {
	...
	while (true) {
		...
		in.get_token();
		if (in.last_token()[0] == ':') { in >> name; } // parent name
		else {
	...
	size_t k;
	for (size_t i = 1; i < _joints.size(); i++) { // find the parent joint of joint i:
		...
		if (k < _joints.size()) {
			_joints[i]->_parent = _joints[k].get();
			_joints[k]->_children.emplace_back(_joints[i].get());
...
static void write_joint(SrOutput& out, SkJoint* j, int marg, const char* geopath, bool writeall) {
	...
	if (writeall) { // write visualization geometry if any :
	...
	if (writeall) { // recurse into children:
...
bool SkSkeleton::export_joints(SrOutput& out) {
	...
	if (_root) // write joints:
	{
		for (size_t i = 0; i < _joints.size(); i++)
			write_joint(out, _joints[i].get(), 0, "", false);
	}
