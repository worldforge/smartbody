...
class SBScene : public SBObject
{
	public:
		...
		/**
		* HACK: executes commands, which are Python scripts. This should be moved out of the libs Scene instance, but we'll keep it for now to avoid updating all scripts.
		*/
...
//		SBAPI void rescalePartialMeshSkeleton(const std::string& meshName, const std::string& skelName, const std::string& rootJointName, const std::vector<std::string>& skipMeshNames, float scaleRatio, float blendThreshold = 0.025f);
	...
	protected:
		...
		/**
		 * HACK: executes scripts. This should be moved out of the libs Scene instance, but we'll keep it for now to avoid updating all scripts.
		 */
