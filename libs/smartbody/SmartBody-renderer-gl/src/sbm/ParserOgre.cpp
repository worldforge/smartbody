...
bool ParserOgre::parse(SmartBody::SBSkeleton& skeleton, std::vector<std::unique_ptr<SmartBody::SBMotion>>& motions, std::string pathName, float scale, bool doParseSkeleton, bool doParseMotion)
{
	try 
	{
		XMLPlatformUtils::Initialize();
	...
	XercesDOMParser skeletonParser;
	skeletonParser.setValidationScheme(XercesDOMParser::Val_Always);
	skeletonParser.setDoNamespaces(true);    // optional

	HandlerBase skeletonErrorHandler;
	skeletonParser.setErrorHandler(&skeletonErrorHandler);
			...
			if (!skeletonNode)
			{
				...
				if (skeletonLinkNode)
				{
					...
					if (nameNode)
					{
						...
						{
							skeletonParser.parse(nameAttr.c_str());
							DOMDocument* skeletonDoc = skeletonParser.getDocument();
							skeletonNode = getNode("skeleton", skeletonDoc);	
							if (skeletonNode)
								foundSkeleton = true;
						}
						else
...
bool ParserOgre::parseMotion(SmartBody::SBScene& scene, DOMNode* animationsNode, std::vector<std::unique_ptr<SmartBody::SBMotion>>& motions, std::string pathName, float scale)
{
	...
	for (unsigned int i = 0; i < animationList->getLength(); i++)
	{
		...
		if (animationName == "animation")
		{
			...
			for (float & allTime : allTimes)
			{
				timeMap.insert(std::pair<float, int>(allTime, counter));
				counter++;
			}
			...
			if (animationAttributeList)
			{
											...
											for (unsigned int m = 0; m < keyframeList->getLength(); m++)
											{
												...
												if (keyframeStr == "keyframe")
												{
													...
													if (keyframeAttributeList)
													{
														...
														auto keyIndexIter = timeMap.find(time);
				...
				{
					// set the key times
					motion->insert_frame(counter, allTime);
					counter++;
				}
				...
				// clean up allData
				for (auto & allDataCounter : allData)
				{
					delete allDataCounter;
					allDataCounter	 = nullptr;
				}
				allData.clear();
			}
			else
