...
//  DWORD line = 0xFFFFFFFF;
  ...
  if ( 1 )
  {
    ...
    if ( SymFromAddr(GetCurrentProcess(), adr, &disp, symbolInfo) != 0 )
    {
      ...
      sprintf(symbol, "%s + 0x%llx", symbolInfo->Name, disp);
...
{
    // under VS2015, MiniDumpWriteDump() isn't found in the VS2015 version of dbghelp.dll.  So, comment out this function until workaround can be found
    // this isn't much of an issue.  We don't use the .dmp files all that often (if ever).
#if 0
  char module[MAX_PATH];
  char drive[MAX_PATH];
  char path[MAX_PATH];
