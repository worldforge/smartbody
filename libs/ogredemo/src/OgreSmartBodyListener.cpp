...
OgreSmartBodyListener::OgreSmartBodyListener(Ogre::SceneManager& sceneManager, SmartBody::SBScene& scene) :
		SBSceneListener(), SBObserver(), mSceneManager(sceneManager), mScene(scene) {
	scene.addSceneListener(this);
}

OgreSmartBodyListener::~OgreSmartBodyListener() {
	mScene.removeSceneListener(this);
}

Ogre::SceneNode* OgreSmartBodyListener::createCharacter(SmartBody::SBCharacter& sbCharacter) {
	...
	*/
	...
	auto skeleton = sbCharacter.getSkeleton();
...
//	}
	...
	for (auto i = 0; i < skeleton->getNumJoints(); ++i) {
		...
		auto jointNode = mSceneManager.createSceneNode();
		auto scaleNode = jointNode->createChildSceneNode();

		auto sphereEntity = mSceneManager.createEntity(Ogre::SceneManager::PT_SPHERE);
		sphereEntity->setMaterial(Ogre::MaterialManager::getSingleton().getDefaultMaterial(true));
		scaleNode->attachObject(sphereEntity);
		SrVec p = joint->getOffset();
		jointNode->setPosition({p.x, p.y, p.z});
		auto q = joint->quat()->value();
		jointNode->setOrientation({q.w, q.x, q.y, q.z});
	...
	for (auto& entry : jointEntries) {
		if (entry.joint.getParent()) {
			auto& parentEntry = jointEntries[entry.joint.parent()->index()];
			parentEntry.node.addChild(&entry.node);
		} else {
	...
	mCharacters.emplace(sbCharacter.getName(), CharacterEntry{*node, sbCharacter, jointEntries});

//	SmartBody::SBAttribute* attribute = sbCharacter.getAttribute("deformableMesh");
//	attribute->registerObserver(this);
	return node;
}

void OgreSmartBodyListener::updateCharacters() {
	for (auto& entry: mCharacters) {


		for (auto& jointEntry: entry.second.jointEntries) {
			auto& joint = jointEntry.joint;
			auto q = joint.quat()->value();
			jointEntry.node.setOrientation({q.w, q.x, q.y, q.z});
		}
		auto pos = entry.second.character.get_world_offset_joint()->pos();
		entry.second.node.setPosition({pos->value(0), pos->value(1), pos->value(2)});
	}
}
...
void OgreSmartBodyListener::OnCharacterDelete(const std::string& name) {
	...
	Ogre::SceneNode* node = (Ogre::SceneNode*) mSceneManager.getRootSceneNode()->getChild(name);
	node->detachAllObjects();
	mSceneManager.destroyEntity(name);
	mSceneManager.getRootSceneNode()->removeAndDestroyChild(name);
}

void OgreSmartBodyListener::OnCharacterUpdate(const std::string& name) {
//	if (!mSceneManager.hasEntity(name)) {
//		SmartBody::util::log("An entity named %s already exists, ignoring...", name.c_str());
//		return;
//	}
...
//									DeformableMesh* neutralMesh = mScene.getAssetManager()->getDeformableMesh(shapeName);
...
//										DeformableMesh* shapeModel = mScene.getAssetManager()->getDeformableMesh(shapeName);
//
//										if (shapeModel) {
...
//			if (ogreCharacter != nullptr) {
//
//				Ogre::SceneNode* node = mSceneManager.getRootSceneNode()->createChildSceneNode("node" + character->getName());
...
//
//	auto charSkel = mScene.getSkeleton(skeletonName);
...
//#endif
//	ogreMesh->load();
//	outChar = mSceneManager.createEntity(sbChar->getName(), meshName);
//
...
void OgreSmartBodyListener::addSBSkeleton(SmartBody::SBSkeleton* skel) {
	...
	if (skelManager.getByName(skelName, Ogre::RGN_DEFAULT)) {
