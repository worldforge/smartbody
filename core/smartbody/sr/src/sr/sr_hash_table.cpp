/*
 *  sr_hash_table.cpp - part of Motion Engine and SmartBody-lib
...
static int hash ( const char* s, int size )
 {
   ...
   // EDF - 4/13/07 Patch to prevent divide by zero crash here.  However, there's question of why this table is being used if the size equals 0.  Need to investigate.
   if ( size == 0 ) return h;
