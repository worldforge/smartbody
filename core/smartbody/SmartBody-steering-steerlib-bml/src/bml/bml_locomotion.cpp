...
#include "bml/bml_xml_consts.hpp"
...
namespace BML {
BehaviorRequestPtr parse_bml_locomotion(DOMElement& elem,
...
//	Locomotion::parse_routine(child, request, type, id);
	...
	if (!steerManager.getEngineDriver()->_engine) {
		SmartBody::util::log("Steering Engine not started. Call \"steer start\" first");
		return BehaviorRequestPtr(new EventRequest(unique_id, localId, "", "", behav_syncs, ""));
	}
...
void Locomotion::parse_routine(DOMElement& elem, BmlRequestPtr request, int type, int id) {
	float speed = 0.0f;
