...
void FLTKOgreWindow::show_viewer()
{
	...
	if (!ogreInterface)
	{
		...
		// initialize the scene with the existing objects
		const std::vector<std::string>& pawnNames = sbScene->getPawnNames();
		for (size_t p = 0; p < pawnNames.size(); p++)
		{
			SmartBody::SBPawn* pawn = sbScene->getPawn(pawnNames[p]);
			SmartBody::SBCharacter* character = dynamic_cast<SmartBody::SBCharacter*>(pawn);
			if (character)
				fltkListener->OnCharacterCreate(pawnNames[p], character->getType() );
			else
				fltkListener->OnPawnCreate(pawnNames[p]);
		}
	}
}
...
void FLTKOgreWindow::updateOgreCamera()
{
	...
	SrVec  p = cam.getEye();
	Ogre::SceneNode* cameraSceneNode = ogreCam->getParentSceneNode();
	cameraSceneNode->setOrientation(Ogre::Quaternion(q.w,q.x,q.y,q.z).Inverse());
	
	cameraSceneNode->setPosition(Ogre::Vector3(p.x,p.y,p.z));			
...
void FLTKOgreWindow::notify(SmartBody::SBSubject* subject)
{
	...
	if (attr)
	{
		if (attr->getObject() == SmartBody::SBScene::getScene())
		{
			...
			if (boolAttribute && boolAttribute->getName() == "scale")
			{
				try {
...
{
	// background color
	int numViewports = ogreInterface->getRenderWindow()->getNumViewports();
	if (numViewports < 1)
		return;
