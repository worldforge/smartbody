...
CEGUI::Key::Scan mapFlKeyToCEGUI(int flKey)
{
	...
	case FL_BackSpace:
		...
		//SmartBody::util::log("flKey = %d, CEGUI Backspace", flKey);
		break;
	case FL_Delete:
		ceguiKey = CEGUI::Key::Delete;
		//SmartBody::util::log("flKey = %d, CEGUI Delete", flKey);
		break;
	case FL_Caps_Lock:
	...
	case FL_Left:
		...
		//SmartBody::util::log("flKey = %d, CEGUI Left", flKey);
		break;
	case FL_Right:
		ceguiKey = CEGUI::Key::ArrowRight;
		//SmartBody::util::log("flKey = %d, CEGUI Right", flKey);
		break;
	case FL_Up:
		ceguiKey = CEGUI::Key::ArrowUp;
		//SmartBody::util::log("flKey = %d, CEGUI Up", flKey);
		break;
	case FL_Down:
		ceguiKey = CEGUI::Key::ArrowDown;
		//SmartBody::util::log("flKey = %d, CEGUI Down", flKey);
		break;
	}
	//SmartBody::util::log("cegui key = %d",ceguiKey);
	return ceguiKey;
}
#endif 
...
{	
	...
	{
		//SmartBody::util::log("Button %d is pushed", button);
		context.injectMouseButtonDown(ceguiButtons[button-1]);				
	}
	else if (eventID == FL_RELEASE) // mouse button release
	{
		//SmartBody::util::log("Button %d is relased", button);
		context.injectMouseButtonUp(ceguiButtons[button-1]);		
	}
	else if (eventID == FL_MOVE || eventID == FL_DRAG) // mouse move
	{	
		//if (eventID == FL_MOVE) SmartBody::util::log("Mouse is moved to %d %d", Fl::event_x(), Fl::event_y());
		//else if (eventID == FL_DRAG) SmartBody::util::log("Mouse is dragged to %d %d", Fl::event_x(), Fl::event_y());
	...
	{
		//SmartBody::util::log("event FL_SHORTCUT");
...
void SBGUIManager::init()
{
	...
	//SmartBody::util::log("After bootstrap system");	
	...
	SmartBody::util::log("setResourceGroupDirectory");
	...
	while ( !pi.isAtEnd () )   {
		...
		if ( curFont ) 
		{
			//SmartBody::util::log("Font name = %s",curFont->getName().c_str());
...
//  	sheet->addChild(quit);
	...
	*/
	...
	//SmartBody::util::log("Finish create CEGUI");	
#endif
}
