...
bool ParserOpenCOLLADA::parse(SkSkeleton& skeleton, SkMotion& motion, std::string pathName, float scale, bool doParseSkeleton, bool doParseMotion)
{
	...
	try 
	{
		...
		if (!skNode)
		{
			SmartBody::util::log("ParserOpenCOLLADA::parse ERR: no skeleton info contained in this file");
			return false;
		}
		...
		if (!skmNode)
		{
		//	SmartBody::util::log("ParserOpenCOLLADA::parse WARNING: no motion info contained in this file");
			return true;
		}
		if (doParseMotion)
	...
	//for (unsigned int i = 0; i < list->getLength(); i++)
	{
		...
		if (nodeName == "node")
		{
			...
			if (treatAsJoint || typeAttr == "JOINT" || (nameAttr.find("Bip")!= std::string::npos && skeleton.root() == nullptr) )
			{
				...
				{
					SmartBody::util::log("Joint name %s already exists, using unique joint id %s instead.", nameAttr.c_str(), sidAttr.c_str());
					joint->name(sidAttr);
...
// 				offset = offset*rotateOffset;
	...
	for (int frameCt = 0; frameCt < motion.frames(); frameCt++)
	{
		for (size_t i = 0; i < quatIndices.size(); i++)
		{
			...
			//SmartBody::util::log("w = %f, x = %f, y = %f, z = %f",quat.w, quat.x, quat.y, quat.z);
		}
...
//	SmartBody::util::log("ParserOpenCOLLADA::getGeometryType WARNING: type %s not supported!", idString.c_str());	
	return "";
}
			...
			//for (unsigned int c1 = 0; c1 < meshNode->getChildNodes()->getLength(); c1++)
			{
				...
				if (nodeName1 == "vertices")
				{
					...
					//for (unsigned int c2 = 0; c2 < node1->getChildNodes()->getLength(); c2++)
					{
						...
						if (XMLString::compareString(inputNode->getNodeName(), BML::BMLDefs::ATTR_INPUTSTR) == 0)
						{
				...
				{
					//SmartBody::util::log("nodeName1 = %s", nodeName1.c_str());
					int curmtl = -1;
					...
					auto mi = std::find(mnames.begin(),mnames.end(), materialID.c_str());
					if ( mi != mnames.end())
					{
						...
						if (XMLString::compareString(inputNode->getNodeName(), BML::BMLDefs::ATTR_INPUTSTR) == 0)
						{
							...
							if (inputMap.find(offset) != inputMap.end())	// same offset is wrong
							{
								if (inputSemantic == "VERTEX" || inputSemantic == "NORMAL" || inputSemantic == "TEXCOORD")
									SmartBody::util::log("ParserOpenCOLLADA::parseLibraryGeometries ERR: file not correct.");
							}
							else
							{
								setModelVertexSource(sourceName,inputSemantic,newModel,floatArrayMap);
					...
					*/
					...
					if (tokens.size() != index)
						SmartBody::util::log("ParserOpenCOLLADA::parseLibraryGeometries ERR: parsing <p> list uncorrectly (%s)!", nameAttr.c_str());
						*/
				}
				meshCurNode = meshCurNode->getNextSibling();
			}
		...
		if (nodeName == "material")
		{
			...
			if (!urlString.empty())
			{
				...
				if (effectId2MaterialId.find(effectId) == effectId2MaterialId.end())
					...
					SmartBody::util::log("ParserOpenCOLLADA::parseLibraryMaterials ERR: two effects mapped to material %s", materialId.c_str());
			}
		}
		curNode = curNode->getNextSibling();
	}
	...
	//for (unsigned int c = 0; c < list->getLength(); c++)
	{
		...
		if (nodeName == "image")
		{
			...
			if (pictureId2File.find(imageId) == pictureId2File.end())
			{
				...
				SmartBody::util::log("ParserOpenCOLLADA::parseLibraryImages ERR: two image files mapped to same image id %s", imageId.c_str());
		}
		curNode = curNode->getNextSibling();
	}
}
...
bool ParserOpenCOLLADA::exportCollada(SmartBody::SBRenderAssetManager& renderAssetManager, std::string outPathname, std::string skeletonName, std::string deformMeshName, std::vector<std::string> motionNames, bool exportSk, bool exportMesh, bool exportMotion, double meshScale )
{	
	if (deformMeshName == "")
	...
	{
		SmartBody::util::log("Folder accessed or created: %s", outPathname.c_str());
	}
	...
	{		
		ParserOpenCOLLADA::exportMaterials(renderAssetManager, fp,deformMeshName);
	...
	if (exportSk)
	{
		...
		ParserOpenCOLLADA::exportVisualScene(renderAssetManager,fp,skeletonName, defMeshName, meshScale);
	}
	fprintf(fp,"</COLLADA>\n");
	fclose(fp);
...
bool ParserOpenCOLLADA::exportMaterials(SmartBody::SBRenderAssetManager& renderAssetManager,  FILE* fp, std::string deformMeshName )
{
	DeformableMesh* defMesh = renderAssetManager.getDeformableMesh(deformMeshName);
	if (!defMesh) 
		return false;
	...
	for (auto & iter : defMesh->morphTargets)
	{
		for (size_t m = 1; m < iter.second.size(); m++) // first morph is the base shape, ignore it
		{
			...
			DeformableMesh* morphMesh = renderAssetManager.getDeformableMesh(morphTarget);
			if (morphMesh)
			{
...
bool ParserOpenCOLLADA::exportGeometry(SmartBody::SBRenderAssetManager& renderAssetManager, FILE* fp, SrModel& model, double scale)
{
	bool hasNormal = false, hasTexCoord = false, hasColor = false;
	if (!model.N.empty() && model.Fn.size() == model.F.size()) hasNormal = true; // export normal
	if (!model.T.empty() && model.Ft.size() == model.F.size()) hasTexCoord = true;
	if (model.Vc.size() == model.V.size()) hasColor = true;

	std::string modelName = (const char*)model.name;

	if (modelName.empty()) // no model name
	{
    ...
    fprintf(fp, "<float_array id=\"%s\" count=\"%lu\">", positionArrayID.c_str(), model.V.size() * 3);
		for (auto & k : model.V)
			fprintf(fp, "%f %f %f ", k[0] * scale, k[1] * scale, k[2] * scale);
    ...
    fprintf(fp, "<triangles material=\"%s\" count=\"%lu\">\n", matID.c_str(), mtlFaces.size());
		...
		fprintf(fp, "<p>");
		for (int fidx : mtlFaces)
		{
				SrVec3i* f = &model.F[fidx];
			SrVec3i *fn = nullptr, *ft = nullptr;
			if (hasNormal)
				fn = &model.Fn[fidx];
			if (hasTexCoord)
				ft = &model.Ft[fidx];
...
bool ParserOpenCOLLADA::exportSkinMesh( SmartBody::SBRenderAssetManager& renderAssetManager, FILE* fp, std::string deformMeshName, double scale, std::string skeletonName)
{
	...
	// export geometry
	for (auto & i : defMesh->dMeshStatic_p)
	{		
		SrModel& model = i->shape();
		exportGeometry(renderAssetManager, fp, model, scale);
	}
	...
	for (auto skinWeight : defMesh->skinWeights)
	{
		...
		for (int j = 0; j < sbSk->getNumJoints(); j++)
		{
			...
			for (auto & k : skinWeight->infJointName)
		{
			auto iter = allJoints.find(k);
		...
		for (const auto & allJoint : allJoints)
		{
			SmartBody::util::log("Joint %s unbound, adding to skinweights...", allJoint.c_str());
		}
		...
		for (auto & k : skinWeight->infJointName)
			fprintf(fp,"%s ",k.c_str());
		// add unmatched joints
		for (const auto & allJoint : allJoints)
		{
			fprintf(fp, "%s ", allJoint.c_str());
		}
		...
		for (auto bpT : skinWeight->bindPoseMat)
		{
		...
		SrMat identity;
		for (auto unmatchedIter = allJoints.begin();
    ...
    fprintf(fp,"<float_array id=\"%s\" count=\"%lu\">", skinWeightArrayID.c_str(), skinWeight->bindWeight.size());
		for (float k : skinWeight->bindWeight)
			fprintf(fp,"%f ",k);
		fprintf(fp,"</float_array>\n");
	...
	for (auto iter = defMesh->morphTargets.begin();		 iter != defMesh->morphTargets.end(); iter++)
	{
...
bool ParserOpenCOLLADA::exportVisualScene( SmartBody::SBRenderAssetManager& renderAssetManager, FILE* fp, std::string skeletonName, std::string defMeshName, double scale )
{
	...
	SmartBody::SBJoint* rootJoint = nullptr;
	if (sbSk)
	{
	...
	if (defMesh)
	{
		...
		if (rootJoint && defMesh->skinWeights.size() > 0)
		{
			...
			for (unsigned int i=0;i<defMesh->skinWeights.size();i++)
			{
				...
				if (model.mtlnames.size() > 0)
				{
					for (auto & mtlname : model.mtlnames)
					{
						matName = mtlname;
						matID = matName+"_SG";
						fprintf(fp,"<instance_material symbol=\"%s\" target=\"#%s\"/>\n",matID.c_str(),matName.c_str());
