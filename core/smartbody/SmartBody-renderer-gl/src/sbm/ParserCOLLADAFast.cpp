...
std::unique_ptr<rapidxml::file<char>> ParserCOLLADAFast::getParserDocumentFile(std::string fileName, rapidxml::xml_document<>* doc)
{
	//std::string filebasename = boost::filesystem::basename(fileName);

	try {
		auto rapidFile = std::make_unique<rapidxml::file<char>>(fileName.c_str());
		doc->parse< rapidxml::parse_declaration_node>(rapidFile->data());
		return rapidFile;
									...
									//for (unsigned int cSource = 0; cSource < realContentNodeList->getLength(); cSource++)
								{
									...
									for (auto & token : tokens)
									{
										...
										if ( isBindJointName && (realNodeName == "Name_array" || realNodeName == "IDREF_array") ) // joint name
										{
											...
											if (token.compare(0, jointPrefix.size(), jointPrefix) == 0)
											{
										...
										{
											bindPosMat[matCounter] = (float)atof(token.c_str());
											matCounter ++;
											if (matCounter == 16)
											{
												matCounter = 0;
...
void ParserCOLLADAFast::parseLibraryVisualScenes(rapidxml::xml_node<>* node, SkSkeleton& skeleton, SkMotion& motion, float scale, int& order, std::map<std::string, std::string>& materialId2Name)
{
	...
	//for (unsigned int c = 0; c < list1->getLength(); c++)
	{
		...
		if (nodeName == "visual_scene")
			parseJoints(node1, skeleton, motion, scale, order, materialId2Name, nullptr);
...
void ParserCOLLADAFast::parseJoints(rapidxml::xml_node<>* node, SkSkeleton& skeleton, SkMotion& motion, float scale, int& order, std::map<std::string, std::string>& materialId2Name, SkJoint* parent, bool hasRootJoint)
{
	...
	//for (unsigned int i = 0; i < list->getLength(); i++)
	{
		...
		if (nodeName == "node")
		{
			...
			if (treatAsJoint || typeAttr == "JOINT" || (nameAttr.find("Bip")!= std::string::npos && skeleton.root() == nullptr) )
			{
				hasRootJoint = true;
				...
				joint->quat()->activate();
				if (skeleton.search_joint(nameAttr.c_str()) != nullptr)
				{
				...
				if (parent == nullptr && !skeleton.root())
					skeleton.root(joint);
		...
		if (node1Name == "animation")
		{
				...
				mi++)
			{
				...
				if (!joint)
				{
					...
					motion->insert_channel(motionChannels.size(), colChannel.sourceName.c_str(), SkChannel::XPos, nullptr);
				}
				if (samplerMap.find(colChannel.sourceName) == samplerMap.end()) continue; // sampler does not exist
				ColladaSamplerFast& sampler = samplerMap[colChannel.sourceName];
				if (floatArrayMap.find(sampler.inputName) == floatArrayMap.end() || floatArrayMap.find(sampler.outputName) == floatArrayMap.end())
				...
				if (motion->frames() == 0)
				{
					for (unsigned int frameCt = 0; frameCt < inFloatArray.floatArray.size(); frameCt++)
					{
						...
						for (int postureCt = 0; postureCt < motion->posture_size(); postureCt++)
							motion->posture(frameCt)[postureCt] = 0.0f;
						// reset quaternion to zero rotation
						for (int k : quatIDList)
						{
							motion->posture(frameCt)[k] = 1.f;
						}
			...
			motion->compress();
			// now there's adjust for the channels by default
			animationPostProcessByChannels(skeleton, motion.get(), channelsForAdjusting);
			//animationPostProcess(skeleton,motion);
...
void ParserCOLLADAFast::animationPostProcess(SkSkeleton& skeleton, SmartBody::SBMotion* motion)
{
	ParserCOLLADAFast::animationPostProcessByChannels(skeleton, motion, motion->channels());
}

void ParserCOLLADAFast::animationPostProcessByChannels(SkSkeleton& skeleton, SmartBody::SBMotion* motion, SkChannelArray& motionChannels)
{
	...
	for (int j = 0; j < numChannel; j++)
	{
		...
		if (!joint)
		{
			jointIndexMap.emplace_back();
			continue;
		}

		int id = motion->channels().search(chanName, chanType);
		...
		jointIndexMap.emplace_back(joint, dataId);
	}
...
void ParserCOLLADAFast::parseLibraryGeometries( rapidxml::xml_node<>* node, const char* file, std::vector<SrMaterial>& M, std::vector<std::string>& mnames, 
	std::map<std::string, std::string>& materialId2Name, std::map<std::string,std::string>& mtlTexMap, std::map<std::string,std::string>& mtlTexBumpMap, std::map<std::string,std::string>& mtlTexSpecularMap, std::map<std::string, std::string>& mtlTransparentMap, std::map<std::string, std::string>& mtlGlossyMap, std::vector<SrModel*>& meshModelVec, float scale )
{
	SrTimer timer;
	timer.start();
	...
	// load up all transparency textures
	std::map<std::string, SbmTexture*> transparentTexMap;
	std::map<std::string, SbmTexture*> glossyTexMap;
	std::map<std::string, std::string>::iterator mi;
	for (mi = mtlTransparentMap.begin();
		mi != mtlTransparentMap.end();
		mi++)
	{
		std::string finalTextureName = getFinalTextureFileName(mi->second, paths);
		if (finalTextureName == "")
			continue;
		SbmTexture* transTex = new SbmTexture("Dummy");
		transTex->loadImage(finalTextureName.c_str());
		transparentTexMap[mi->first] = transTex;
	}

	for (mi = mtlGlossyMap.begin();
		mi != mtlGlossyMap.end();
		mi++)
	{
		std::string finalTextureName = getFinalTextureFileName(mi->second, paths);
		...
		glossyTexMap[mi->first] = glossyTex;
	}
...
void ParserCOLLADAFast::setModelVertexSource( std::string& sourceName, std::string& semanticName, SrModel* model, VecListMap& vecMap )
{
	...
	std::vector<SrVec>* sourceArray = nullptr;
	if (srcName[0] == '#') 
	{
		srcName.erase(0,1);		
	}
...
void ParserCOLLADAFast::parseLibraryEffects( rapidxml::xml_node<>* node, std::string assetName, std::map<std::string, std::string>&effectId2MaterialId, std::map<std::string, std::string>& materialId2Name, std::map<std::string, std::string>& pictureId2File, std::map<std::string, std::string>& pictureId2Name, std::vector<SrMaterial>& M, std::vector<std::string>& mnames, std::map<std::string,std::string>& mtlTexMap, 
											std::map<std::string,std::string>& mtlTexBumpMap, std::map<std::string,std::string>& mtlTexSpecularMap, std::map<std::string, std::string>& mtlTransparentMap, std::map<std::string, std::string>& mtlGlossyMap)
{
	//const DOMNodeList* list = node->getChildNodes();
	rapidxml::xml_node<>* curNode = node->first_node();
	while (curNode)
	//for (unsigned int c = 0; c < list->getLength(); c++)
	{
		...
		if (nodeName == "effect")
		{
			...
			std::string diffuseTexture, normalTexture, specularTexture, transparentTexture, glossyTexture;
			diffuseTexture = "";
			normalTexture = "";
			specularTexture = "";
			glossyTexture = "";
			...
			rapidxml::xml_node<>* displacementNode = ParserCOLLADAFast::getNode("displacement", node);
			rapidxml::xml_node<>* shininessNode = ParserCOLLADAFast::getNode("shininess", node);
			...
			if (shininessNode)
			{
				float shininessVal = 20.f; // default shininess
				rapidxml::xml_node<>* floatNode = ParserCOLLADAFast::getNode("float", shininessNode);
				if (floatNode)
				{
					shininessVal = (float)atof(floatNode->value());
				}
				M.back().shininess = shininessVal;
				rapidxml::xml_node<>* texNode = ParserCOLLADAFast::getNode("texture", shininessNode);
				if (texNode)
				{
					rapidxml::xml_attribute<>* texAttrNode = texNode->first_attribute("texture");
					std::string texID = texAttrNode->value();
			...
			for (unsigned int i = 0; i<initNodes.size(); i++)			// IF there's init_from node inside library_effect 
			{
				...
				else if (transparentTexture.find(imageName) != std::string::npos)
...
{
	rapidxml::xml_document<> doc;
	auto rapidFile = ParserCOLLADAFast::getParserDocumentFile(fileName, &doc);
	if (!rapidFile)
	{
	...
	if (geometryNode)
	{
		...
		if (effectNode)
		{
			ParserCOLLADAFast::parseLibraryEffects(effectNode,"unknown", effectId2MaterialId, materialId2Name, pictureId2File, pictureId2Name, M, mnames, mtlTextMap, mtlTextBumpMap, mtlTextSpecularMap, mtlTransparentMap, mtlGlossyMap);
		}
		// parsing geometry
		ParserCOLLADAFast::parseLibraryGeometries(geometryNode, fileName.c_str(), M, mnames, materialId2Name, mtlTextMap, mtlTextBumpMap, mtlTextSpecularMap, mtlTransparentMap, mtlGlossyMap, meshModelVecs, 1.0f);
	}
	else
	{
