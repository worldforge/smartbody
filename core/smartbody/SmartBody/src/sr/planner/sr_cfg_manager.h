...
class SrCfgManagerBase : protected SrClassManagerBase
 { 
   ...
   public : // virtual callbacks
    ...
    virtual void sr_free ( srcfg* c )=0;
    ...
    /*! The apply_time( float t ) method is called everytime a motion->apply(t)
        is called; only applicable in a time-varying problem */
//    virtual void apply_time ( float t )=0;
 };
...
class SrCfgManager : public SrCfgManagerBase, public M
 { public :
    ...
    virtual float time ( srcfg* c )
     { return M::time ( (C*)c ); }
//    virtual void apply_time ( float t )
//     { M::apply_time(t); }
 };
