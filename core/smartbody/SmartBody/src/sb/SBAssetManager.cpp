...
//std::vector<std::unique_ptr<SBAsset>> SBAssetManager::loadAsset(const std::string& assetPath)
//{
...
//
//		const std::string& convertedPath = pathname.string();
//#ifdef WIN32
//		boost::replace_all(convertedPath, "\\", "/");
//#endif
//		if (ext == ".skm" || ext == ".SKM") {
//			auto* motion = new SmartBody::SBMotion();
...
//int SBAssetManager::load_me_motion_individual( SrInput & input, const std::string & motionName, std::map<std::string, std::unique_ptr<SBMotion>>& map, double scale )
//{
//	auto motion = std::make_unique<SmartBody::SBMotion>();
//
//	bool parseSuccessful = motion->load( input, scale );
//
//	const std::string& filename = motionName;
//
...
//		motion->setName( filebase );
//	}
//
//	motion->filename( motionName.c_str() );
//
//	auto motionIter = map.find(filebase);
...
//	}
//
//	map.emplace(filebase, std::move(motion));
//
//	return CMD_SUCCESS;
//}
//
...
//
//
//	auto iter =_skeletons.find(std::string(skel_file));
//	if (iter != _skeletons.end())
//	{
//		//SkSkeleton* ret = new SkSkeleton(iter->second);
//		auto& existingSkel = iter->second;
//		auto* ret = new SmartBody::SBSkeleton(existingSkel.get());
//		ret->ref();
//		return ret;
//	}
//
...
//int SBAssetManager::load_me_skeletons_impl( const std::string& pathStr, std::map<std::string, std::unique_ptr<SmartBody::SBSkeleton>>& map, bool recurse_dirs, double scale, const char* error_prefix )
//{
//	boost::filesystem::path pathname(pathStr);
//	if( !exists( pathname ) ) {
...
//
//
//int SBAssetManager::load_me_skeleton_individual( SrInput & input, const std::string & skeletonName, std::map<std::string, std::unique_ptr< SmartBody::SBSkeleton>>& map, double scale )
//{
...
//		skeleton->setFileName(skeletonName);
//		map.emplace(skeleton->getName(), std::move(skeleton));
//	}
...
//}
//
//int SBAssetManager::load_me_skeletons( const char* pathname, std::map<std::string, std::unique_ptr<SmartBody::SBSkeleton>>& map, bool recurse_dirs, double scale ) {
//	boost::filesystem::path motions_path(pathname);
//
//	boost::filesystem::path finalPath;
...
//	std::vector<std::unique_ptr<SBAsset>> assets = this->loadAsset(neutralShapeFile);
//	for (size_t a = 0; a < assets.size(); a++)
//	{
...
//	SbmTexture* neutralTexture = nullptr;
//
//	if (neutralMesh)
//	{
...
//	SbmTexture* expressiveTexture = nullptr;
//
//	if (expressiveMesh)
//	{
