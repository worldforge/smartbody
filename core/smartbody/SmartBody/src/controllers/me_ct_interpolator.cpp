...
void MeCtInterpolator::initKeys()
{
	...
	if (key1.size() != key2.size() || key1.size() == 1 || key2.size() == 1)
	{
		key1.clear();
		key2.clear();
		SmartBody::util::log(" MeCtInterpolator::initKeys() : Key map mismatching !!!");
	}
...
{
	if (key1.empty() || key2.empty())
		SmartBody::util::log(" MeCtInterpolator::initDuration() : Key map not initialized.");
...
void MeCtInterpolator::getTiming(double t, double& t1, double& t2)
{
	...
	if (err)	
	{
		SmartBody::util::log("MeCtInterpolator::getTiming : Check the Timing.");
		return;
	}
...
void MeCtInterpolator::adjustStartTime(double origW, double newW)
{
	...
	if (err)	
	{
		SmartBody::util::log("MeCtInterpolator::adjustStartTime : Check the Timing.");
		return;
	}
