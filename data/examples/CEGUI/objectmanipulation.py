...
class GUIHandler:
    ...
    def handleGrabButton(self, args):
        ...
        if (not onAction and not grabbed):
            ...
            delay = 2
            baseName = 'object%s' % currentPawnNum
            bml.execBML('ChrBrad1',
                        '<sbm:reach sbm:action="pick-up" sbm:reach-finish="true" sbm:reach-duration="2" target="' + baseName + '" sbm:use-locomotion="true"/>')
    ...
    def handleGrabMoveTopButton(self, args):
        ...
        if (not onAction and grabbed and not released):
            ...
            canOtherAction = False
            released = True
            bml.execBML('ChrBrad1', '<sbm:reach sbm:action="put-down" sbm:reach-finish="true" sbm:target-pos="' + '%s %s %s' % (
            4.9, 1.6, 4.9) + '" sbm:use-locomotion="true"/>')
            bml.execBML('ChrBrad1', '<gaze target="' + baseName + '" sbm:joint-range="EYES NECK"/>')
            delay = 3
            onAction = True
    ...
    def handleGrabMoveMidButton(self, args):
        ...
        if (not onAction and grabbed and not released):
            ...
            released = True
            bml.execBML('ChrBrad1', '<sbm:reach sbm:action="put-down" sbm:reach-finish="true" sbm:target-pos="' + '%s %s %s' % (
            4.9, 0.9, 4.9) + '" sbm:use-locomotion="true"/>')
            bml.execBML('ChrBrad1', '<gaze target="' + baseName + '" sbm:joint-range="EYES NECK"/>')
            delay = 3
            onAction = True
    ...
    def handleGrabMoveBotButton(self, args):
        ...
        if (not onAction and grabbed and not released):
            ...
            released = True
            bml.execBML('ChrBrad1', '<sbm:reach sbm:action="put-down" sbm:reach-finish="true" sbm:target-pos="' + '%s %s %s' % (
            4.9, 0.3, 4.9) + '" sbm:use-locomotion="true"/>')
            bml.execBML('ChrBrad1', '<gaze target="' + baseName + '" sbm:joint-range="EYES NECK"/>')
            delay = 3
            onAction = True
