...
class PerlinNoiseDemo(SBScript):
    def update(self, time):
        ...
        if timeDiff >= 5:
            ...
            # Perlin noise
            if which == 0:
                bml.execBML('ChrBrad0',
                            '<gesture lexeme="DEICTIC" type="YOU" poststroke_hold="2" sbm:joint-range="l_shoulder l_elbow" sbm:frequency="0.05" '
                            'sbm:scale="0.02"/>')
                bml.execBML('ChrBrad1', '<gesture lexeme="DEICTIC" type="YOU" poststroke_hold="2"/>')
            elif which == 1:
                bml.execBML('ChrBrad0',
                            '<gesture lexeme="METAPHORIC" type="OBLIGATION" mode="LEFT_HAND" poststroke_hold="2" sbm:joint-range="l_shoulder l_elbow" '
                            'sbm:frequency="0.05" sbm:scale="0.02"/>')
                bml.execBML('ChrBrad1', '<gesture lexeme="METAPHORIC" type="OBLIGATION" mode="LEFT_HAND" poststroke_hold="2"/>')
            elif which == 2:
                bml.execBML('ChrBrad0',
                            '<gesture lexeme="DEICTIC" type="ME" poststroke_hold="2" sbm:joint-range="l_shoulder l_elbow" sbm:frequency="0.05" sbm:scale="0.02"/>')
